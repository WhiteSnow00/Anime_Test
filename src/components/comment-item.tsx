  ...
  const loadAllReplies = async () => {
    ...
    try {
      const response = await fetch(
        `/api/comments/reply?commentId=${comment._id}`
      );
      const data = await response.json();
    ...
    <>
      ...
      >
        ...
        <div className="flex items-start gap-2 sm:gap-3">
          ...
          <div
            className={cn(
              "w-9 h-9 sm:w-10 sm:h-10 rounded-full flex items-center justify-center flex-shrink-0 transition-all ring-1",
              comment.userRole === "administrator"
                ? "bg-red-100 dark:bg-red-950/40 ring-red-500/30"
                : "bg-red-50/40 dark:bg-red-950/15 ring-red-200/30"
            )}
          >
            {comment.userRole === "administrator" ? (
              <Snowflake className="h-4 w-4 sm:h-5 sm:w-5 text-red-600 dark:text-red-400" />
            ) : comment.userId ? (
              <Squirrel className="h-4 w-4 sm:h-5 sm:w-5 text-red-500/70 dark:text-red-300/70" />
            ) : (
              <Turtle className="h-4 w-4 sm:h-5 sm:w-5 text-red-600/80 dark:text-red-400/80" />
            )}
          </div>
          ...
          <div className="flex-1 min-w-0">
              ...
              <div>
                <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-2">
                  <div className="flex items-center gap-2">
                    <span className="font-medium text-[0.95rem] comment-username vietnamese-text">
                  ...
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    <div className="flex items-center gap-1 text-xs text-muted-foreground">
                      ...
                      {CommentService.formatRelativeTime(
            ...
            <CommentContent
              ...
              className="text-[0.95rem] sm:text-base leading-relaxed vietnamese-text mb-4"
            />
            ...
            {showReplyForm && isLoggedIn && (
              <div className="mt-4 space-y-3">
                <div className="flex items-start gap-2">
                  <div
                    className={cn(
                      "w-8 h-8 sm:w-9 sm:h-9 rounded-full flex items-center justify-center flex-shrink-0 ring-1",
                      user?.role === "administrator" &&
                        "bg-red-100 dark:bg-red-950/40 ring-red-500/30",
                      user &&
                        user?.role !== "administrator" &&
                        "bg-red-50 dark:bg-red-950/30 ring-red-400/30"
                    )}
                  >
                    {user?.role === "administrator" ? (
                      <Snowflake className="h-4.5 w-4.5 text-red-600 dark:text-red-400" />
                    ) : (
                      <Turtle className="h-4.5 w-4.5 text-red-600/80 dark:text-red-400/80" />
                    )}
                  </div>
...
function ReplyItem({
  ...
  return (
    <>
      <div
        className={cn(
          "flex items-start gap-3 bg-muted/25 rounded-lg p-3 sm:p-3.5 hover:bg-muted/35 transition-all relative",
      ...
      >
        ...
        <div
          className={cn(
            "w-8 h-8 sm:w-9 sm:h-9 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5 ring-1",
        ...
        >
          {reply.userRole === "administrator" ? (
            <Snowflake className="h-3.5 w-3.5 text-red-600 dark:text-red-400" />
          ) : reply.userId ? (
            <Turtle className="h-4 w-4 text-red-600/80 dark:text-red-400/80" />
          ) : (
            <Squirrel className="h-4 w-4 text-red-500/70 dark:text-red-300/70" />
          )}
        </div>
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between">
            <div className="flex items-center flex-wrap gap-2 mb-1.5">
              <span className="font-medium text-[13px] sm:text-sm text-foreground/90">
                {reply.displayName || reply.userName}
              </span>
              {reply.userId && (
                <Badge
                  variant={
                    reply.userRole === "administrator" ? "outline" : "secondary"
                  }
                  className={cn(
                    "text-[11px] sm:text-xs py-0 px-1.5 h-5 flex items-center border",
                    !reply.userId &&
          ...
          <div className="text-[0.9rem] leading-relaxed text-foreground/85 mb-2">
            {reply.replyTo && (
