...
export function JWPlayerComponent({
  ...
  const isMobileDevice = () => {
    ...
    return (
      hasTouch && (mobileRegex.test(userAgent.toLowerCase()) || isSmallScreen)
    );
  };
  ...
  useEffect(() => {
    ...
    if (!isMobile) {
      ...
      const hideCustomContextMenu = (e: MouseEvent) => {
        if (
          customContextMenu &&
          !customContextMenu.contains(e.target as Node)
        ) {
          customContextMenu.remove();
            ...
            if (prop === "setAttribute") {
              return function (
                ...
                if (attr === "src" && value.includes("blob:")) {
                  return HTMLVideoElement.prototype.setAttribute.call(
                    this,
                    attr,
                    value
                  );
                ...
                for (let i = 0; i < target.length; i++) {
                  ...
                  const proxiedElement =
                    element.tagName === "VIDEO"
                      ? createVideoElementProxy(element as HTMLVideoElement)
                      : element;
      ...
      document.querySelector = function <E extends Element>(
        selectors: string
      ): E | null {
      ...
      document.querySelectorAll = function <E extends Element>(
        selectors: string
      ): NodeListOf<E> {
      ...
      const createDecoyElements = () => {
        for (let i = 0; i < 3; i++) {
          ...
          decoyVideo.src = `blob:decoy-${Math.random()
            .toString(36)
            .substring(7)}`;
      ...
      console.log(
        "Mobile device detected - protection features disabled for performance"
      );
    }
  }, []);
  ...
  const initializePlayer = useCallback(async () => {
    ...
    if (playerInstanceRef.current && playerInstanceRef.current._isFullscreen) {
      console.log(
        "Skipping player initialization due to player fullscreen flag"
      );
      return;
    }
    ...
    try {
      ...
      player.on("ready", () => {
        ...
        if (
          ...
          playerInstanceRef.current._savedPosition
        ) {
        ...
        setTimeout(() => {
          ...
          if (videoElement) {
            try {
              ...
              videoElement.addEventListener("contextmenu", (e) => {
                ...
                const existingMenu =
                  document.querySelector("#kana-custom-menu");
                if (existingMenu) {
                  existingMenu.remove();
                }
          ...
          if (playerInstanceRef.current) {
            try {
              ...
              const originalGetPlaylistItem =
                playerInstanceRef.current.getPlaylistItem;
      ...
      player.on("error", (e: any) => {
        ...
        if (e.sourceError)
          errorDetails += `Source: ${JSON.stringify(e.sourceError)}, `;

        console.log("Error details:", errorDetails);

        let errorMessage = `Failed to load video: hls - ${videoId} (JWPlayer error: ${
      ...
      player.on("setupError", (e: any) => {
        ...
        const errorMessage = `Lỗi thiết lập player: ${
          e.message || "Khởi tạo player thất bại"
        }`;
        setError(errorMessage);
        setIsLoading(false);
        onError?.(errorMessage);
  ...
  useEffect(() => {
    if (playerLoaded && playerInstanceRef.current) {
      ...
      if (
        isFullscreen ||
        (playerInstanceRef.current && playerInstanceRef.current._isFullscreen)
      ) {
      ...
      const now = Date.now();
      if (
        playerInstanceRef.current._lastFullscreenExit &&
        now - playerInstanceRef.current._lastFullscreenExit < 2000
      ) {
        console.log(
          "Skipping player reinitialization - recently exited fullscreen"
        );
        return;
      }
