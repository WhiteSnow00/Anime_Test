...
export default function JWPlayerNew({ videoUrl }: { videoUrl: string }) {
  const addSkipButtons = (player: any) => {
    player.addButton(
  ...
  useEffect(() => {
    ...
    const getPlayerSetupConfig = () => ({
      ...
      sources: [
        {
          file: videoUrl,
          type: videoUrl?.match(/m3u8/) ? "hls" : "mp4",
        },
      ],
      ...
      aboutlink: "https://ayaya-kana.id.vn/",
    });

    const initializePlayer = () => {
      if (!window.jwplayer) return;

      playerInstance = window.jwplayer(playerId);
      playerInstance.setup(getPlayerSetupConfig());
      addSkipButtons(playerInstance);
    };
